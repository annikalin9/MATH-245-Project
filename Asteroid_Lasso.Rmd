...
set.seed(1)
cvfit = cv.glmnet(x=X[,-1], y=Y, family="multinomial", type.measure="class")
plot(cvfit)
```
```{r}
...
```{r}
#using the first matrix from cvfit$lamda.1se
sel.vars <- which(coef(cvfit, s=cvfit$lambda.1se)$'0'!=0)[-1]-1
...
#confused about error: variable lengths differ (found for 'Absolute.Magnitude')
df$Y = as.numeric(df$Hazardous=="True")

fit.lasso <- glm(as.formula(paste(df$Y, 
