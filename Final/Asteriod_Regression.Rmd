...
X = model.matrix(Hazardous ~ ., data=nasa_outlier) 
...
sel.names <- colnames(nasa_outlier)[sel.vars]
sel.names
```
...
Due to a large outlier (observation 695), none of the assumptions for regression are met. Therefore, we remove line 695 and begin the analysis again.
...
#create the design matrix
X2 = model.matrix(Hazardous ~ ., data=nasa) 
Y2 = as.numeric(nasa$Hazardous=="True")
...
sel.names2 <- colnames(nasa)[sel.vars2]
sel.names2
```
...
fit.lasso3 <- glm(as.factor(Hazardous) ~ Est.Dia.in.KM.min. + Orbit.Uncertainity +
              Minimum.Orbit.Intersection + Mean.Motion,family="binomial", data=nasa)
summary(fit.lasso3)
```
